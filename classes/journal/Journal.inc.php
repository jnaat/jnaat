...
class Journal extends DataObject {
	 ...
	 * @return string
	 */
	function getLocalizedTitle($issue = null, $preferredLocale = null) {
		if($issue != null)
		{
			$title="";
			$year = (int) $issue->getYear();
			if($year <= 1974)
				$title="Rev. Anal. Numér. Théorie Approximation";
			else
				if($year <= 1991)
					$title="Anal. Numér. Théor. Approx.";
				else
					if($year <= 2014)
						$title="Rev. Anal. Numér. Théor. Approx.";
					else
						$title="J. Numer. Anal. Approx. Theory";
			return $title;
		}
		else

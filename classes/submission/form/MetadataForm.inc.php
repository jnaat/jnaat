...
class MetadataForm extends Form {
	...
	function MetadataForm($article, $journal) {
		...
		if ($this->canEdit) {
			...
			$this->addCheck(new FormValidatorArrayCustom($this, 'authors', 'optional', 'author.submit.form.authorRequiredFields', create_function('$email, $regExp', 'return String::regexp_match($regExp, $email);'), array(ValidatorEmail::getRegexp()), false, array('email')));
